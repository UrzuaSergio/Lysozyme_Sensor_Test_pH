...
rot_N = 36  #valor es 36 por ejemplo
...
for i in range(len(til_angles)):
    ...
    cmd_move = 'python move_protein_z-axis.py ' + prot_file + ' ' + pqr_file + ' ' + str(int(rot_angles[i])) + ' ' + str(int(til_angles[i])) +' '+ str(H) + ' ' + name
    os.system(cmd_move)

    #ejecutar pygbe
    cmd_run = ' /user/s/surzua/PyCuda/pycuda/bin/pygbe '+ Nombre_carpeta_principal +' -p' + param_file + ' -c' + config_file_moved +' > output_aux_' + output_file + name
    os.system(cmd_run)
